...
### バグ修正: 翻訳の無効化・空状態で注入済み翻訳が残る問題の修正
- 翻訳機能を無効化した場合、翻訳ファイルがない場合、または有効な翻訳エントリがない場合に、注入済み翻訳をクリアする処理を追加
- `EnableTranslation` の切り替え時に翻訳を再読み込みするように変更
- 既存の注入ソースを置き換える際、追跡リストからも削除して古いソース参照が残らないように修正
- 翻訳辞書のフィンガープリント計算をキー順で決定的にし、実行環境や列挙順に依存しない判定に変更
- 抽出ファイル書き込みが空パスを返した場合に失敗扱いとし、部分失敗を成功扱いしないように修正

### ドキュメント・配布設定
- 抽出済み翻訳ファイルの配置例を `Translations/<ModName>/<ModName>.json` 形式に更新
- `.gitattributes` を追加し、ソース・Markdown・JSON・YAML の改行コードを LF に正規化
- `.gitignore` に `progress.yaml` を追加
